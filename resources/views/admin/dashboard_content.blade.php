    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mt-4">
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-slate-200/50 p-8 lg:p-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <a href="{{ route('admin.books.index') }}" class="group bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white p-6 rounded-2xl text-center transition-all duration-300 hover:shadow-xl hover:scale-[1.02] border border-blue-500/20">
                ...
                <a href="{{ route('admin.books.create') }}" class="group bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white p-6 rounded-2xl text-center transition-all duration-300 hover:shadow-xl hover:scale-[1.02] border border-emerald-500/20">
                    <svg class="w-12 h-12 mx-auto mb-4 opacity-90 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-end">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-slate-900">{{ rand(2450,3500) }}<span class="text-xl text-emerald-600 ml-1">+12%</span></p>
                        </div>
                        <div class="p-4 bg-white/50 backdrop-blur-sm rounded-2xl text-indigo-600 font-bold text-2xl">{{ rand(100,500) }} Baru</div>
                    </div>
                    <div class="h-2 bg-slate-200 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-indigo-500 to-blue-500 rounded-full w-{{ rand(60,85) }}%" style="width: {{ rand(60,85) }}%;"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-teal-50 p-8 rounded-3xl shadow-xl border border-emerald-100">
                ...
                <div class="space-y-4 text-sm">
                    <div class="flex justify-between py-2 border-b border-emerald-100 last:border-b-0">
                        ...
                        <span class="font-bold text-emerald-700">{{ rand(250,800) }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-emerald-100 last:border-b-0">
                        <span>Peminjaman Hari Ini</span>
                        <span class="font-bold text-emerald-700">{{ rand(25,75) }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-emerald-100 last:border-b-0">
                        <span>Terlambat</span>
                        <span class="font-bold text-orange-600">{{ rand(0,8) }}</span>
                    </div>
                    <div class="flex justify-between py-2">
                        <span>Total Riwayat</span>
                        <span class="font-bold text-slate-800">{{ rand(5000,15000) }}</span>
                    </div>
                </div>
            </div>
        </div>
